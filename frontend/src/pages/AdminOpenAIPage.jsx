...
export default function AdminOpenAIPage() {
  ...
  const fetchConfig = async () => {
    try {
      const res = await fetch(`${API}/api/admin/openai/config`, { headers: authHeaders() });
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/admin/openai/config`, { method: "POST", headers: authHeaders(), body: JSON.stringify(body) });
      if (res.ok) {
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      const res = await fetch(`${API}/api/admin/openai/test`, { method: "POST", headers: authHeaders() });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#6B7280]">{config?.api_key_set ? `Configurata (${config.api_key_preview})` : "Non configurata"}</p>
              </div>
            </div>
