...
export default function MerchantReferredUsersPage() {
  ...
  const fetchData = async (q = "") => {
    try {
      const res = await fetch(withBackendPath(`/api/merchant/referred-users?search=${q}`), { headers: authHeaders() });
  ...
  const handleExport = () => {
    window.open(withBackendPath("/api/merchant/referred-users/export"), "_blank");
  };
