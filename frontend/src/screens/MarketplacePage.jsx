...
// API
import { merchantAPI, MERCHANT_CATEGORIES, myuAPI } from "@/lib/api";
...
export default function MarketplacePage() {
  ...
  const loadLocationState = async () => {
    ...
    try {
      const state = await myuAPI.getLocation();
      setLocationState(state);
      return state;
    } catch (err) {
      console.error("Marketplace location fetch error:", err);
      return null;
    } finally {
  ...
  const fetchData = async (locationOverride) => {
    setLoading(true);
    try {
      const nextLocation = locationOverride === undefined ? await loadLocationState() : locationOverride;
      const merchantsData = await merchantAPI.getAll({
        city: nextLocation?.inferred_city || null,
        prioritizeCity: Boolean(nextLocation?.inferred_city),
      });
  ...
  const getGeolocationErrorMessage = (error) => {
    if (error?.code === 1) return "Permesso posizione negato. Mostro tutti i merchant.";
    if (error?.code === 2) return "Posizione non disponibile al momento.";
    if (error?.code === 3) return "Richiesta posizione scaduta. Riprova.";
    return "Non sono riuscito a rilevare la posizione.";
  };

  const handleEnableLocation = async () => {
    if (!navigator.geolocation) {
      const message = "Questo dispositivo non supporta la geolocalizzazione.";
    ...
    try {
      ...
      const updatedLocation = await myuAPI.updateLocation(
      ...
      await fetchData(nextLocationState);
      if (updatedLocation.inferred_city) {
        toast.success(`Merchant ordinati per ${updatedLocation.inferred_city}`);
      } else {
        toast.success("Posizione salvata. Ti mostro tutti i merchant.");
      }
    } catch (err) {
      const message = err?.code ? getGeolocationErrorMessage(err) : (err.message || "Errore posizione");
      setLocationError(message);
      toast.error(message);
    } finally {
  ...
  const nearbyCity = locationState?.inferred_city || null;
  const getMerchantCity = (address = "") => {
  ...
  const isNearbyMerchant = (merchant) =>
    Boolean(nearbyCity) && getMerchantCity(merchant.address).includes(nearbyCity.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-white pb-safe">
      ...
      <div className="px-6 pt-8 pb-4">
        ...
        <p className="text-[#6B7280] mb-4">Scopri i merchant affiliati</p>

        <div className="mb-6 rounded-2xl border border-[#2B7AB8]/15 bg-[#F4F9FD] p-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-[#1A1A1A]">
                {nearbyCity ? `Merchant vicino a ${nearbyCity}` : "Mostra i merchant vicino a te"}
              </p>
              <p className="mt-1 text-sm text-[#6B7280]">
                {nearbyCity
                  ? "La posizione viene chiesta qui e usata per ordinare prima i merchant della tua zona."
                  : "Consenti la posizione in questa pagina per vedere prima i merchant della tua zona."}
              </p>
              {locationError && (
                <p className="mt-2 text-xs font-medium text-[#E85A24]">{locationError}</p>
              )}
            </div>
            <Button
              onClick={handleEnableLocation}
              disabled={locationRequesting || locationLoading}
              className="h-11 rounded-full bg-[#2B7AB8] px-4 text-sm font-semibold hover:bg-[#236699]"
            >
              <MapPin className="w-4 h-4" />
              {locationRequesting ? "Attendo..." : nearbyCity ? "Aggiorna" : "Attiva"}
            </Button>
          </div>
        </div>
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    {isNearbyMerchant(merchant) && (
                      <span className="text-xs px-2 py-1 bg-[#E85A24]/15 text-[#E85A24] rounded-full">
                        Vicino a te
                      </span>
                    )}
