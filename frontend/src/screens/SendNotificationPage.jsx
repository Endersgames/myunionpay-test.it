...
import { 
  ...
  MapPin, Globe, Eye, User, Tag, Image, X, Megaphone, Calendar
} from "lucide-react";
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await notificationAPI.sendMerchantNotification({
        ...
        validity_minutes: formData.validity_minutes,
        target_tags: formData.target_tags,
        target_cap: formData.target_cap,
        target_all_italy: formData.target_all_italy,
      });
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#F5F5F5] rounded-2xl p-5 border border-black/5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <Calendar className="w-5 h-5 text-[#E85A24]" />
                <Label htmlFor="validity" className="text-[#1A1A1A] font-semibold">Validita Reward</Label>
              </div>
              <span className="font-mono text-lg font-bold text-[#E85A24]">
                {formatValidity(formData.validity_minutes)}
              </span>
            </div>
            <Input
              id="validity"
              ...
              onChange={(e) => {
                const nextValue = Math.max(1, Math.min(2160, Number(e.target.value) || 1));
                setFormData(prev => ({ ...prev, validity_minutes: nextValue }));
              }}
              className="h-12 bg-white border-black/10 focus:border-[#2B7AB8] rounded-xl text-[#1A1A1A]"
              data-testid="validity-minutes-input"
            />
            <div className="flex justify-between text-xs text-[#6B7280] mt-2">
              <span>Min 1 minuto</span>
              <span>Max 36 ore</span>
            </div>
            <p className="text-xs text-[#6B7280] mt-3">
              Il reward resta prenotato fino alla lettura. Se scade senza accredito torna nel wallet merchant.
            </p>
          </div>
          ...
          <p className="text-center text-xs text-[#6B7280]">
            L'importo viene bloccato dal tuo wallet e accreditato solo agli utenti che leggono entro {formatValidity(formData.validity_minutes)}
          </p>
        </div>
      </div>
    </div>
