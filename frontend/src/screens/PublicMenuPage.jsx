...
const getMerchantIdFromLocation = () => {
...
export default function PublicMenuPage() {
  ...
  const { user } = useAuth();
  const [resolvedMerchantId, setResolvedMerchantId] = useState(() => normalizeRouteParam(merchantId));
  ...
  useEffect(() => {
    const nextMerchantId = normalizeRouteParam(merchantId) || getMerchantIdFromLocation();
    if (nextMerchantId && nextMerchantId !== resolvedMerchantId) {
      setResolvedMerchantId(nextMerchantId);
    }
  ...
  useEffect(() => {
    ...
    setLoading(true);
    menuAPI.getPublicMenu(resolvedMerchantId).then((d) => {
      setData(d);
  ...
  const effectiveReferralCode = ref || merchant.referral_code || "";
  const buildRegisterUrl = () => {
    const params = new URLSearchParams();
    if (effectiveReferralCode) params.set("ref", effectiveReferralCode);
    if (merchant?.id) params.set("redirect", `/menu/${merchant.id}`);
