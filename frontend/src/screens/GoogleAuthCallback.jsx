...
import { authAPI, clearAuth, setAuthToken } from "@/lib/api";
...
const GOOGLE_ERROR_MESSAGES = {
  access_denied: "Accesso Google annullato.",
  callback_processing_failed: "Errore interno durante il callback Google. Riprova.",
  google_email_missing: "Google non ha restituito un indirizzo email valido.",
  google_not_configured: "Accesso Google non configurato correttamente.",
  google_upstream_unavailable: "Servizio Google temporaneamente non disponibile. Riprova.",
  invalid_state: "Sessione Google non valida. Riprova dall'inizio.",
  missing_access_token: "Accesso Google incompleto. Riprova.",
  missing_code_or_state: "Callback Google incompleto. Riprova.",
  token_exchange_failed: "Scambio credenziali Google fallito. Riprova.",
  userinfo_failed: "Recupero profilo Google fallito. Riprova.",
};

function getGoogleErrorMessage(errorCode) {
  return GOOGLE_ERROR_MESSAGES[errorCode] || "Errore autenticazione Google";
}

async function ensureUserLoadedAfterGoogle({
  refreshUser,
  onFailure,
  logContext,
}) {
  ...
  console.error("[Google Auth] auth/me failed after callback", logContext);
  clearAuth();
  onFailure();
  return null;
}
...
export default function GoogleAuthCallback() {
  ...
  useEffect(() => {
    ...
    const processCallback = async () => {
      const pendingContext = getPendingGoogleAuthContext();
      ...
      if (callbackError) {
        clearPendingGoogleAuthContext();
        console.error("[Google Auth] Callback returned error", { callbackError });
        toast.error(getGoogleErrorMessage(callbackError));
      ...
      if (!sid) {
        clearPendingGoogleAuthContext();
        console.error("[Google Auth] Callback missing session_id");
      ...
      try {
        ...
        if (result.is_new) {
          ...
          setGoogleData({
            ...result,
            redirect_path: result.redirect_path || pendingContext.redirect_path || "",
            referral_code: result.referral_code || pendingContext.referral_code || "",
          });
          setNeedsPhone(true);
          setLoading(false);
        } else {
          ...
          const userData = await ensureUserLoadedAfterGoogle({
            refreshUser,
            logContext: { sessionId: sid, userId: result.user_id, flow: "existing_user" },
            onFailure: () => {
              clearPendingGoogleAuthContext();
              toast.error("Accesso Google completato, ma il profilo non è disponibile. Riprova.");
              navigate("/login", { replace: true });
            },
          });
          if (!userData) {
          ...
          navigate(result.redirect_path || pendingContext.redirect_path || "/dashboard", { replace: true });
        }
      } catch (err) {
  ...
  const handlePhoneSubmit = async (e) => {
    ...
    try {
      const result = await authAPI.googleComplete(sessionId, trimmed, {
        redirect: googleData?.redirect_path || undefined,
        referral_code: googleData?.referral_code || undefined,
      });
      setAuthToken(result.token);
      localStorage.setItem("user_id", result.user_id);
      const userData = await ensureUserLoadedAfterGoogle({
        refreshUser,
        logContext: { sessionId, userId: result.user_id, flow: "new_user_complete" },
        onFailure: () => {
          clearPendingGoogleAuthContext();
          toast.error("Registrazione Google completata, ma il profilo non è disponibile. Riprova.");
          navigate("/login", { replace: true });
        },
      });
      if (!userData) {
