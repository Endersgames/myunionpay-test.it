...
export default function AdminUsersPage() {
  ...
  const fetchUsers = async (q = search) => {
    try {
      const res = await fetch(withBackendPath(`/api/admin/users?search=${q}&status=${filter}`), { headers: authHeaders() });
  ...
  const handleBlock = async (userId, block) => {
    try {
      const res = await fetch(withBackendPath(`/api/admin/user/${userId}/${block ? "block" : "unblock"}`), { method: "POST", headers: authHeaders() });
      if (res.ok) {
  ...
  const openEdit = async (userId) => {
    try {
      const res = await fetch(withBackendPath(`/api/admin/user/${userId}`), { headers: authHeaders() });
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(withBackendPath(`/api/admin/user/${editUser.id}`), {
