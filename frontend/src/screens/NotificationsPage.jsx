...
const MYU_EMPTY_MESSAGES = [
  "Qui troverai novita, menu e idee da tenere d'occhio.",
  "Per ora tutto calmo, ma appena spunta qualcosa di curioso te lo porto qui.",
  "Niente di nuovo adesso. Appena arriva qualcosa di buono, ti avviso.",
];

const MYU_COMMENTS = [
  "Qui potrebbe esserci qualcosa di nuovo.",
  "Questo posto mi incuriosisce.",
  "Dagli un'occhiata, magari c'e una novita interessante.",
  "Vediamo se nasconde qualcosa di buono.",
];
...
function NotificationCard({ notif, onRead, onClick, onCtaClick, expanded, onToggle }) {
  ...
  const rewardStatus = notif.reward_status || (notif.reward_amount > 0 ? "credited" : "credited");
  const isExpired = notif.is_expired || rewardStatus === "refunded";
  const isCredited = rewardStatus === "credited";
  const rewardLabel = isExpired ? "Scaduta" : isCredited ? "Accreditato" : "Da leggere";
  const rewardLabelClass = isExpired
    ? "text-[#FF3B30]"
    : isCredited
      ? "text-emerald-600"
      : "text-[#2B7AB8]";
        ...
        <div className="flex-1 min-w-0">
          ...
          {notif.reward_amount > 0 && (
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className={`flex items-center gap-0.5 text-[10px] ${rewardLabelClass}`}>
                {isCredited ? <Check className="w-3 h-3" /> : null}
                {rewardLabel}
              </span>
            </div>
          )}
        ...
        <div className="px-4 pb-4 pt-0 border-t border-black/5 mt-0" style={{ animation: "myuFadeIn 0.3s ease" }}>
          ...
          {notif.reward_amount > 0 && (
            <div className="mt-3 text-xs text-[#6B7280]">
              {isExpired ? (
                <span>Reward scaduto e tornato al merchant.</span>
              ) : isCredited ? (
                <span>Reward accreditato il {formatDate(notif.credited_at || notif.read_at || notif.created_at)}.</span>
              ) : (
                <span>Leggi entro {formatAbsoluteDate(notif.expires_at)} per ricevere il reward.</span>
              )}
            </div>
          )}
...
export default function NotificationsPage() {
    ...
    try {
      ...
      setNotifications(prev => prev.map((n) => (
        n.id === id
          ? {
              ...n,
              is_read: result.is_read ?? true,
              read_at: result.read_at ?? n.read_at,
              credited_at: result.credited_at ?? n.credited_at,
              reward_status: result.reward_status ?? n.reward_status,
              is_expired: result.expired ?? n.is_expired,
            }
          : n
      )));
    } catch {}
  ...
  const unreadCount = notifications.filter(
    (n) => !n.is_read && !n.is_expired && n.reward_status !== "refunded",
  ).length;
