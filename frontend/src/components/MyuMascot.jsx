...
const GLOBAL_RANDOM_TIP = "Come va oggi la tua energia ?";
...
export default function MyuMascot() {
  ...
  const [startupIdleComplete, setStartupIdleComplete] = useState(false);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setStartupIdleComplete(true), INITIAL_IDLE_MS);
  ...
  useEffect(() => {
    if (isHidden || showInstallBubble || !startupIdleComplete) {
      setShowTip(false);
      setCurrentAnim("");
      return undefined;
    }
    ...
    const firstTip = window.setTimeout(() => {
      setTipText(GLOBAL_RANDOM_TIP);
      setShowTip(true);
      const hideTimer = window.setTimeout(() => setShowTip(false), TIP_VISIBLE_MS);
      timers.push(hideTimer);
    }, INITIAL_IDLE_MS);
    const initialTip = window.setTimeout(showNextTip, TIP_INITIAL_DELAY_MS + INITIAL_IDLE_MS);
    const recurringTips = window.setInterval(showNextTip, TIP_ROTATION_DELAY_MS);
    timers.push(firstTip, initialTip, recurringTips);

    return () => timers.forEach((timer) => window.clearTimeout(timer));
  }, [isHidden, pageConfig, showInstallBubble, startupIdleComplete]);
