...
from models.myu import CoachingProfileUpdate, CoachingProfileResponse
from services.auth import get_current_user
from myu.orchestrator import handle_chat, MYU_COST_PER_MSG
from myu.coaching_profile import get_user_coaching_profile, upsert_user_coaching_profile
from myu.coaching_planner import build_coaching_plan
...
@router.get("/coaching-profile")
async def get_coaching_profile(user=Depends(get_current_user)):
    """Get the authenticated user's MYU coaching profile."""
    profile = await get_user_coaching_profile(user["id"])
    return {"profile": CoachingProfileResponse(**profile).model_dump()}


@router.put("/coaching-profile")
async def update_coaching_profile(data: CoachingProfileUpdate, user=Depends(get_current_user)):
    """Create or update the authenticated user's MYU coaching profile."""
    profile = await upsert_user_coaching_profile(user["id"], data.model_dump(exclude_unset=True))
    return {
        "success": True,
        "profile": CoachingProfileResponse(**profile).model_dump(),
    }


@router.get("/proactive/signals")
async def get_proactive_signals(user=Depends(get_current_user)):
    """Get MYU proactive triggers (task due, inactivity, milestones, upcoming events)."""
    bundle = await collect_proactive_signals(user["id"], max_signals=6)
    return bundle


@router.post("/coaching-plan")
async def generate_coaching_plan(data: CoachingPlanRequest, user=Depends(get_current_user)):
    """Generate a realistic MYU coaching plan based on profile + compensation knowledge."""
    profile = await get_user_coaching_profile(user["id"])
    goal_text = (data.financial_goal or "").strip() or profile.get("economic_goal", "") or ""
    ...
    query_text = goal_text or "piano compensi"
    try:
        ...
        logger.warning(f"Compensation retrieval failed for coaching-plan endpoint: {exc}")

    plan = build_coaching_plan(
        user_profile=profile,
        financial_goal=goal_text,
